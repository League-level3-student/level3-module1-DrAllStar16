package _03_IntroToStacks;

import java.util.Random;
import java.util.Stack;

import javax.swing.JFrame;
import javax.swing.JOptionPane;
import javax.swing.JPanel;

public class _01_IntroToStack {

	public static void main(String[] args) {
		double dfirst = 0;
		double dsecond = 0;
		JFrame f = new JFrame();
		JPanel p = new JPanel();
		f.add(p);
		f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		f.pack();
		// 1. Create a Stack of Doubles
		// Don't forget to import the Stack class
		Stack<Double> thing = new Stack<Double>();
		// 2. Use a loop to push 100 random doubles between 0 and 100 to the Stack.
		Random r = new Random();

		for (int i = 0; i <= 100; i++) {
			double d = r.nextDouble() * 100;
			thing.push(d);
		}
		// 3. Ask the user to enter in two numbers between 0 and 100, inclusive.
		String first = JOptionPane.showInputDialog("Pick a number between 0 and 100");
		String second = JOptionPane.showInputDialog("Pick a number between 0 and 100");
		dfirst = Double.parseDouble(first);
		dsecond = Double.parseDouble(second);

		// 4. Pop all the elements off of the Stack. Every time a double is popped that
		// is
		// between the two numbers entered by the user, print it to the screen.
		for (int i = 0; thing.size() > 0; i++) {
			double pop = thing.pop();
			if (pop >= dfirst && pop <= dsecond) {
				System.out.println(pop);
			}
		}
		System.out.println(thing.size());

		// EXAMPLE:
		// NUM 1: 65
		// NUM 2: 75

		// Popping elements off stack...
		// Elements between 65 and 75:
		// 66.66876846
		// 74.51651681
		// 70.05110654
		// 69.21350456
		// 71.54506465
		// 66.47984807
		// 74.12121224
	}
}
